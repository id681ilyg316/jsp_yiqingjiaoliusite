...
<table width="100%" class="table">  
 ...
 <tr>
  ...
  <% 
    ...
    String url = "bingrenguiji_list2.jsp?1=1"; 
    String sql =  "select * from bingrenguiji where tianjiaren='"+request.getSession().getAttribute("username")+"' ";
...
if(request.getParameter("fengxian")=="" ||request.getParameter("fengxian")==null ){}else{sql=sql+" and fengxian like '%"+request.getParameter("fengxian")+"%'";}
  ...
  <tr>
    ...
    <td width="220" align="center"><a href="bingrenguiji_updt.jsp?id=<%=map.get("id")%>"class="A1">修改</a>  <a href="bingrenguiji_list.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')"class="A2">删除</a>  <a href="bingrenguiji_detail.jsp?id=<%=map.get("id")%>" class="A3">详细</a> </td>
  </tr>  	<%
  }
   %>
  </tbody> 
